...
function renderHome(){current.view="home";const pp=phonicsProgress();const ma=mathsAccuracy();shell(`<div class="screen"><div class="topbar"><div class="brand"><span class="brand-mark">🌟</span><span>BrightSteps 2.1</span></div><div class="status-row"><span class="pill streak">🔥 ${state.streak}</span><span class="pill">⭐ ${state.stars}</span><span class="pill">💎 ${state.gems}</span></div></div>
...
<div class="cards"><button class="card phonics" data-action="phonics"><span class="card-icon">🌳</span><h3>Phonics Forest</h3><p>Hear pure UK sounds, recognise graphemes and blend decodable words.</p><div class="card-meta"><div class="progress-track"><span style="width:${pp}%"></span></div><span>${pp}%</span></div></button>
<button class="card maths" data-action="maths"><span class="card-icon">⛰️</span><h3>Maths Mountain</h3><p>Thirty adaptive questions, read aloud in a British voice, across counting and reasoning.</p><div class="card-meta"><div class="progress-track"><span style="width:${ma}%"></span></div><span>${ma||0}% accuracy</span></div></button>
...
function renderTrails(){current.view="trails";reconcileTrailProgress();const cards=TRAILS.map((t,i)=>{const unlocked=i<=state.unlockedTrail;const completed=state.completedTrails.includes(i);const vals=t.sounds.map(s=>state.phonics[s]||0);const mastered=vals.filter(v=>v>=3).length;return `<button class="trail ${unlocked?"":"locked"}" data-trail="${i}" ${unlocked?"":"disabled"}><span class="trail-number">${completed?"✓":i+1}</span>${unlocked?"":"<span class='lock'>🔒</span>"}<h3>${t.emoji} ${t.name}</h3><div class="sound-chips">${t.sounds.map(s=>`<span class="sound-chip">${s}</span>`).join("")}</div><div class="mastery">${completed?"Trail completed · ":""}${mastered}/${t.sounds.length} mastered</div></button>`}).join("");shell(`<div class="screen">${header("Phonics Forest","A structured journey from first sounds to digraphs.")}<div class="panel"><strong>How progression works</strong><p class="helper">Complete every activity in a full trail to unlock the next path. Repeating a trail continues building each sound towards three-step mastery.</p></div><div class="trail-grid">${cards}</div></div>`)}
