...
function renderGarden(effect=""){
  ...
  const roamingAnimals=state.garden.animals.map((item,index)=>{const info=ANIMALS[item.type];const lane=index%3;const delay=-(index*3.7);const duration=11+(index%4)*2.5;return `<button class="roaming-animal roaming-lane-${lane}" data-select-animal="${esc(item.type)}" style="--wander-delay:${delay}s;--wander-time:${duration}s" aria-label="${esc(info.name)}"><span>${info.emoji}</span><small>${esc(info.name)}</small></button>`;}).join("");
