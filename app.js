...
function renderGarden(effect=""){
  ...
  const roamingAnimals=state.garden.animals.map((item,index)=>{const info=ANIMALS[item.type],selected=item.type===state.garden.selectedAnimal;const lane=index%4;const delay=-(index*4.1);const duration=22+(index%4)*4;return `<button class="roaming-animal roaming-lane-${lane} ${selected?"selected":""}" data-select-animal="${esc(item.type)}" style="--wander-delay:${delay}s;--wander-time:${duration}s" aria-label="${esc(info.name)}"><span class="animal-face"><span class="animal-sprite">${info.emoji}</span></span><small>${esc(info.name)}</small></button>`;}).join("");
  ...
  shell(`<div class="screen garden-screen ${effect}">${screenHeader("My Garden","Everything you finish growing stays here.")}
    <section class="garden-world ${effect}"><div class="garden-sky"><span>☀️</span><i>☁️</i></div><div class="garden-roaming-animals">${roamingAnimals}</div><div class="garden-collection">${permanent||"<p class='empty-garden'>Your first plant will appear here when it is fully grown.</p>"}</div><div class="garden-ground"></div></section>
